                        ...
                        <ul class="menu-list menu-list-block" id="sidebarUl">
                            <li class="menu-list-item">
                                <a
                                    class="menu-link"
                                    @click="
                                        setReport(reportSample1, null, 'viewer')
                                    "
                                >
                                    <span>리포트 샘플1</span>
                                </a>
...
export default {
    ...
    methods: {
        setReport(report, data, type) {
            this.$emit('setReportHandler', report, data, type);
        },
        setCompositeReport(reports, datas) {
            const formSets = [
                {
                    form: reports[0],
                    dataSet: null,
                },
                { form: reports[1], dataSet: datas },
            ];
            this.$emit('setCompositeReportHandler', formSets);
        },
    },
};
</script>
