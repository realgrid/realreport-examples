...
function setPrintDocument (win, report, cssFiles, afterClose, immediatelyPrint) {
    ...
    if (cssFiles)
        cssFiles.map(function (css) {
            win.document.write("<link href='".concat(css, "' media=' rel='stylesheet' /></head>"));
...
if (typeof window !== 'undefined')
    window['r2printer'] = {
        // 기본 템플릿으로 print
        popup: function (report, cssFiles, afterClose, immediatelyPrint) {
            var printWindow = window.open('#', 'printWindow', 'status=1,width=800px,height:1280px');
            if (printWindow) {
        ...
        popupTemplate: function (url, report, afterClose, immediatelyPrint) {
            var printWindow = window.open(url, 'printWindow', 'status=1,width=800px,height:1280px');
            if (printWindow) {
